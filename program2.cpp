...
int main()
{
	...
	int x;
	cin >> x;
	if (cin.fail()) //проверяем ввел ли пользователь целое число
{
   cerr << "Некорректный ввод числа, попробуйте снова!\n";
}
else{
        if (x<9999 & x>1) //проверяем ввел ли пользователь значение от 1 до 9999
        {
	cout << "Итого: ";
		switch (x / 1000) //проверяем на тысячи
		{
		case 1:
    ...
    if (x/1000>=1) //на сотни если число оказалось сколько-то тысяч
        {
            switch(x/100)
            {
	...
	switch(x / 100) //на сотни если без тысяч
	{
		case 1:
			cout << "сто "; break;
		case 2:
	...
	{
		switch (x % 100) //на числа от 10 до 19
		{
		case 10:
			cout << "десять ";break;
		case 11:
	...
	if (x % 100 >= 11 && x % 100 <= 19) //тут и ниже проверяем какое окончание добавить к слову "рубль"
	{
		cout << "рублей\n";
	}
	else
	{
        ...
        cout<<"Число должно быть в диапазоне от 1 до 9999!\n";
	}
